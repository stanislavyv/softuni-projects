...
const moviesService = (() => {
    ...
    const addMovie = async function (userId, movieData) {
        if (!movieData.title || !movieData.description || !movieData.imageUrl) {
            userAuth.showError('Please fill in all required fields!');
            return false;
        }

        try {
            requester.post(userId, movieData);
            userAuth.showInfo(`Successfully added ${movieData.title}`);
            return true;
        } catch (e) {
            userAuth.showError(`There was an error while adding the movie - ${e.message}`);
            return false;
        }
    };
    ...
    const likeMovie = async function (userId, movieId, currLikes) {
        try {
            ...
            userAuth.showError(`There was an error while trying to like this movie - ${e.message}`);
            return false;
        }
    };
    ...
    const editMovie = async function (userId, movieId, movieData) {
        ...
        try {
            ...
            userAuth.showInfo(`Successfully edited ${movieData.title}`);
            return true;
        } catch (e) {
            userAuth.showError(`There was an error while editing the movie - ${e.message}`);
            return false;
        }
    }
    ...
    const deleteMovie = async function (userId, movieId) {
        try {
            ...
            userAuth.showError(`Coundn\'t delete movie - ${e.message}`);
            return false;
        }
    }
    ...
    const searchMovie = async function (userId, movieTitle) {
        ...
        try {
            ...
            userAuth.showError(`Couldn't find movie - ${e.message}`);
            return false;
        };
    }
