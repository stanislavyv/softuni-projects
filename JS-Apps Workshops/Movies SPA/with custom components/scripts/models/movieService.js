...
const movieService = (() => {
    ...
    const addMovie = async function (userId, movieData) {
        if (!validator.movieData(movieData)) {
        ...
        try {
            ...
            notify('success', `Successfully added ${movieData.title}`);
            return true;
        } catch (e) {
            notify('error', `There was an error while adding the movie - ${e.message}`);
            return false;
        }
    };

    const likeMovie = async function (userId, movieId) {
        try {
            const movie = await getMovie(userId, movieId);
            let currLikes = Number(movie.likes ? movie.likes : 0);
            ...
            notify('error', `There was an error while trying to like this movie - ${e.message}`);
            return false;
        }
    };
    ...
    const editMovie = async function (userId, movieId, movieData) {
        ...
        try {
            ...
            notify('success', `Successfully edited ${movieData.title}`);
            return true;
        } catch (e) {
            notify('error', `There was an error while editing the movie - ${e.message}`);
            return false;
        }
    }
    ...
    const deleteMovie = async function (userId, movieId) {
        try {
            ...
            notify('error', `Coundn\'t delete movie - ${e.message}`);
            return false;
        }
    }
    ...
    const searchMovie = async function (userId, movieTitle) {
        ...
        try {
            ...
            notify('error', `Couldn't find movie - ${e.message}`);
            return false;
        };
    }
